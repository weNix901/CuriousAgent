...
> 一个具有**内在好奇心**和**元认知能力**的自主探索 Agent。不是等待用户提问，而是主动发现知识缺口、智能评估探索价值、持续积累、精准分享。
...
### 🧠 v0.2.2 元认知监控器 (Metacognitive Monitor)

Curious Agent v0.2.2 引入元认知层，解决"无限循环"和"价值缺失"问题：
...
┌─────────────────────────────────────────────────────────┐
│  Monitor（监控层）                                       │
│  • should_explore()    → 检查探索次数上限 (max 3次)      │
│  • assess_quality()    → 三维质量评分 (0-10)             │
│  • compute_marginal()  → 边际收益计算                    │
└─────────────────────────┬───────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────┐
│  Verify（验证层）                                        │
│  • should_notify()     → 高质量才通知 (≥7.0)             │
│  • should_continue()   → 边际收益低则停止                │
│  • EventBus.emit()     → 触发事件通知                    │
└─────────────────────────────────────────────────────────┘
```
...
```json
{
  ...
  "min_marginal_return": 0.3,    // 最小边际收益
...
| 特性 | 说明 |
|------|------|
| ✅ **元认知监控器** | MetaCognitiveMonitor - 三维质量评分 (0-10) |
| ✅ **MGV 循环架构** | Monitor-Generate-Verify 闭环控制 |
| ✅ **智能探索阻止** | 单话题最多探索 3 次，防止无限循环 |
| ✅ **边际收益计算** | 自动检测探索价值衰减，及时停止 |
| ✅ **质量阈值通知** | 仅高质量发现 (≥7.0) 触发用户通知 |
| ✅ **事件总线** | EventBus 解耦通知机制 |
| ✅ **多 LLM 路由** | LLMManager 支持多 Provider 负载均衡 |
| ✅ **元认知 API** | `/api/metacognitive/*` 完整状态查询 |
| ✅ **Web UI 面板** | 实时显示元认知状态和已完成话题 |
| ✅ **48项测试覆盖** | 核心模块 100% 单元测试 |
...
_最后更新：2026-03-21 | v0.2.2_
_设计理念：好奇驱动，主动探索，元认知调控，以我为名_

📄 **Release Notes**: [v0.2.2](./RELEASE_NOTE_v0.2.2.md) | [v0.2.1](./RELEASE_NOTE_v0.2.1.md) | [v0.2.0](./RELEASE_NOTE_v0.2.0.md)

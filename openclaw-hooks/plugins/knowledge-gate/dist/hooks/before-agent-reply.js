...
exports.beforeAgentReplyHook = void 0;
...
async function queryConfidence(topic) {
    try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 1000);
        const response = await fetch(`${CA_API}/api/kg/confidence/${encodeURIComponent(topic)}`, { headers: COMMON_HEADERS, signal: controller.signal });
        clearTimeout(timeout);
        return response.ok ? await response.json() : null;
    }
    catch {
...
const beforeAgentReplyHook = async ({ context }) => {
    try {
        ...
        if (confidence >= 0.85) {
            contextParts.push(`[KG Context — 置信度高 ${(confidence * 100).toFixed(0)}%]\n` +
                `KG 有完整知识。`);
        }
        else if (confidence >= 0.6) {
            contextParts.push(`[KG Context — 置信度中 ${(confidence * 100).toFixed(0)}%]\n` +
                `KG 有部分知识，建议搜索补充。`);
        }
        else if (confidence > 0) {
            contextParts.push(`[KG Context — 置信度低 ${(confidence * 100).toFixed(0)}%]\n` +
                `KG 知识有限。`);
        }
...
exports.beforeAgentReplyHook = beforeAgentReplyHook;
